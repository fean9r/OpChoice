...
class Scheduler():
    def __init__(self, constraints):
        self.scheduler_constraints = constraints
        self.act_LPname_map = {}
        self.activity_list = []
...
#         self.max_num_film_day = max_film_for_day
    ...
    def __writeProblem(self, activity_list):
        ...
        for i in range(n):
            name = "x_%d" % i
            self.act_LPname_map[name] = activity_list[i]       
        ...
        for i in range(n):
            ...
            condition = pulp.lpSum([dot_SameWeek_x]) <= self.scheduler_constraints.max_activity_by_week
        ...
        for i in range(n):
            ...
            condition = pulp.lpSum([dot_SameDay_x]) <= self.scheduler_constraints.max_activity_by_day
            self.lp_prob += condition, label
